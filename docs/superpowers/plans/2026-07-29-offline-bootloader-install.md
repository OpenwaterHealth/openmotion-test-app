...
7. **Two DFU devices at once** — added after the final review. Strand one sensor in DFU (start an install and let it abort), then attempt a console bootloader install while both are enumerated as `0483:df11`. Expect: nothing is written, and the install fails with a dfu-util "More than one DFU capable USB device found" error. This is the **only** point in the flow where the fail-safe rests on dfu-util's own behavior rather than on this app's code — `wait_for_dfu_device` succeeds with two devices present, and `parse_boot_mode` can read `BARE_METAL` off the wrong one, so `install_bootloader`'s "already installed" guard can be satisfied by a device that is not the target. Verify this empirically rather than trusting the reasoning.
